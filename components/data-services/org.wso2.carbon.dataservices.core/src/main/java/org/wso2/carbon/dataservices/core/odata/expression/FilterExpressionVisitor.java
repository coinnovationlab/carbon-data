...
public class FilterExpressionVisitor implements ExpressionVisitor<Object> {
	 ...
	 * by the visitBinaryOperator method, which will also add brackets before and after the string generated by this query, making the sub-query complete.
	 * 
	...
	private String whereSubQuery(String dbType, String foreignTable, SubQueryType queryType, Expression lambdaExpression) throws ExpressionVisitException, ODataApplicationException {
		...
		if (queryType == SubQueryType.COUNT) { // filter on the amount of related entities
			preSubQuery = String.join("," + tableName + ".", tablePKs);
			preSubQuery += ") IN ("; // note the ), which closes brackets that have not been opened yet: as described above, the opening bracket (and table prefix) will be added by the visitBinaryOperator method
		} else if (queryType == SubQueryType.ALL) {
			preSubQuery = "(" + tableName + "." + String.join("," + tableName + ".", tablePKs);
		...
		for (ForeignKey fk : toForeignTableFKs) {
			if (addJoin) { // add join command
				rdbmsQuery.appendFrom(fk.getDatabase() + "." + tableName + " LEFT OUTER JOIN " + fk.getForeignDatabase() + "." + foreignTable + " ON ");
				addJoin = false;
